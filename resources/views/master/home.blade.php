...
@section('content')
    <div class="container">
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link active" href="{{ route('master.index') }}">ข้อมูลสมาชิกทั้งหมด</a>
            </li>
            <li class="nav-item">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-list">
                                    ...
                                    @foreach ($headers as $index => $header)
                                        <tbody>
                                            <tr>
                                                <td width="7%"><img class="img-rounded img-responsive center-block"
                                                        src="{{ url('uploads') }}/{{ $header->image }}" width="100%"></td>
                                                <td class="hidden-xs">{{ $NUM_PAGE * ($page - 1) + $index + 1 }}</td>
                                                <td>
                                                    @if ($header->status == '1')
                                                        <font color="green"> &#9679; </font>{{ $header->user_name }}
                                                    @else
                                                        <font color="red"> &#9679; </font>{{ $header->user_name }}
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="{{ url('master/show/') }}/{{ $header->id }}"
                                                        class="btn btn-info btn-sm"><i class="fa fa-eye"></i> แสดง</a>
                                                    <a href="{{ url('master/edit/') }}/{{ $header->id }}"
                                                        class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i>
                                                        แก้ไข</a>
                                                    {{-- @if ($header->status == '1')
                                                        <form action="{{ url('master/block') }}/{{ $header->id }}"
                                                            onsubmit="return myFunction();return false;" method="post">
                                                        ...
                                                        @else
                                                        <a href="{{ url('master/block') }}/{{ $header->id }}"
                                                            class="btn btn-danger btn-sm"
                                                            onclick="return confirm('ท่านต้องการปลดบล็อกสมาชิกใช่หรือไม่ ?')"><i
                                                                class="fa fa-trash"></i>ปลดบล็อก</a>
                                                    @endif
                                                    {{ csrf_field() }} --}}
                                                </td>
